...
Options:
  ...
  -l, --logfile        path to write log file to (default cli_log_$PID.txt)
...
ZooKeeper client output is written to "cli_log_%d.txt" (or the supplied filename) where %d is the process id of the python process.
